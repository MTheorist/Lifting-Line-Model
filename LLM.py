...
def BladeElementMethod(Vinf, TSR, n, rho, b, r, root_pos_R, tip_pos_R, dr, Omega, Nb, a, a_tan, twist, chord, polar_alfa, polar_cl, polar_cd, tol, P_up):
    ...
    return a_b4_Pr, a, a_tan, Cl, Cd, F_ax, F_tan, alfa, phi, F_tot, F_tip, F_root, dCT, dCQ, dCP, P0_down, Gamma, V_loc
...
def LiftingLineModel(HS_vortex, CtrlPts, polar_alfa, polar_cl, polar_cd, Vinf, Omega, rho, b, r_R, chord_dist, twist_dist, Nb, l, U_wake, vor_fil, gamma, a_bem, alfa):
    ...
    gamma_new, alfa, phi, a_ax_loc, a_tan_loc, F_ax_l, F_tan_l, r_cp, dCT, dCQ, dCP, seg_len, V_local_mag = [np.zeros(N_cp) for i in range(13)]
    results = {'r':[], 'a_ax':[], 'a_tan':[], 'alfa':[], 'phi':[], 'F_ax':[], 'F_tan':[], 'CT': 0.0, 'CQ': 0.0, 'CP': 0.0, 'Gamma':[], 'V_mag':[], 'iterations':0}
    ...
    while error>conv and iter<max_iter:
        ...
        for i in range(N_cp):
            ...
            V_local_mag[i] = np.sqrt((V_ax_local**2) + (V_tan_local**2))
            ...
            Lift_loc = 0.5 * rho * (V_local_mag[i]**2) * Cl_local * chord_local
            Drag_loc = 0.5 * rho * (V_local_mag[i]**2) * Cd_local * chord_local 

            F_ax_loc = (Lift_loc * np.cos(phi[i])) - (Drag_loc * np.sin(phi[i]))
            F_tan_loc = (Lift_loc * np.sin(phi[i])) + (Drag_loc * np.cos(phi[i]))

            gamma_new[i] = 0.5 * V_local_mag[i] * Cl_local * chord_local 
...
chord, a_tan, Cl, Cd, F_ax, F_tan, dCT, dCQ, dCP, alfa, phi, F_tot, F_tip, F_root, P0_down, Gamma, V_loc = [np.zeros((len(J),len(r_R)-1)) for i in range(17)]
...
for j in range(len(J)):
    for i in range(len(r_R)-1):    
        ...
        a_b4_Pr[j][i], a[j][i], a_tan[j][i], Cl[j][i], Cd[j][i], F_ax[j][i], F_tan[j][i], alfa[j][i], phi[j][i], F_tot[j][i], F_tip[j][i], F_root[j][i], dCT[j][i], dCQ[j][i], dCP[j][i], P0_down[j][i], Gamma[j][i], V_loc[j][i] = \
